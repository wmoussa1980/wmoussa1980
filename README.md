...
In this session, we learned about file handling in programming languages. Files are named locations on disk to store related information. They are used to permanently store data in a non-volatile memory. File handling is an integral part of programming. File handling in Python is simplified with built-in methods, which include creating, opening, and closing files. The key function for working with files in Python is the open() function. The open() function takes two parameters; filename, and mode. We can specify the mode while opening a file. In mode, we specify whether we want to read r, write w or append a to the file. We can also specify if we want to open the file in text mode or binary mode. The default is reading in text mode. In this mode, we get strings when reading from the file. While files are open, Python additionally allows performing various file operations, such as reading, writing, and appending information.

## Assignment 15: Final project

In this final project, I used the knowledge I earned through all the semester. First, I was overthinking how to do this program, but following the instruction the professor gave in the weekly announcements, I realized that to solve this program I have to follow what I have learned all semester. The steps I took to do this final project is first to read the xml file using the open() function takes two parameters; filename, and mode read. And then parse the lines to get the data I needed, put it into an array. After creating five parallel arrays which is required, I used the for loop to display the array items. I think that programming is not just about learning the language. It is about problem solving and the skills I will learned in this programming class is applicable in every field and everyday life. Learning to break a problem into parts, consider it from multiple aspects, finding the most efficient solution, error identifying and paying attention to details are good skills to have.
